...
export interface IServerToClientEvent<T = unknown> {
  ...
  'action:call-accepted': Ev<{ to?: string; from?: string }>
  'action:incoming-call': Ev<{ from: string }>
  'action:call-ended': Ev<{ message: string }>
}
...
export interface IClientToServerEvent<T = unknown> {
  ...
  'request:call-user': Ev<{ message?: string; from: string; to: string }>
  'request:accept-call': Ev<{ from: string }>
  'request:reject-call': Ev<{ from: string }>
}
